...
# 2019-07-21 (C) woodie2wopper 

programname <- "fft2spectrogram.R";
...
if( length(args) > 0) {
  ...
  output_image_body 	<- "190801_060000-070000"
  output_spgrm_param 	<- paste0(output_image_body,".psgrm");
  input_fft_param 		<- paste0(output_image_body,".pfft");
  input_fft_file 		<- paste0(output_image_body,".fft");
  ...
  color_opt <- "color"
  ratio_clipping <- 4; 
  threshold_clipping_lo <- -250 # default = -200; #dB
  threshold_clipping_hi <- -100 # default = -100; #dB
...
# 周波数フィルタの設定 # HCFの設定周波数がデータの持つ最大周波数より大きければ何もしない
